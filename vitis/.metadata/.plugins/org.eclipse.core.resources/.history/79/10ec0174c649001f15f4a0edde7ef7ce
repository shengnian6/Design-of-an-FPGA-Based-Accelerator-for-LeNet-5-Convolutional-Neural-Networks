...
void cnn_infer()
{
	...
	for(int img_index=0; img_index<1; img_index++){
        uint8 right = test_label[img_index];
        int p = Predict(lenet, test_data[img_index], 10);
        // if(p != right) printf("fail: predict = %0d, label = %0d\n", p, right);
        // else           printf("success: predict = %0d, label = %0d\n", p, right);
	}
